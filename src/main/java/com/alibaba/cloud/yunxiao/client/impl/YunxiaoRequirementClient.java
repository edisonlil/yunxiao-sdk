 ...
 */
@Slf4j
public class YunxiaoRequirementClient extends AbstractYunxiaoClient implements RequirementClient {

    public YunxiaoRequirementClient(YunxiaoProperties properties) {
        super(properties);
    }
    ...
    @Override
    public BaseResponse<RequirementInfo> getRequirement(String requirementId) {
        try {
            String url = buildApiUrl("/workitems/" + requirementId);
            
            HttpResponse response = sendRequest("GET", url, null);
            return parseResponse(response, RequirementInfo.class);
        } catch (Exception e) {
            log.error("获取需求信息失败，需求ID: {}", requirementId, e);
            return buildErrorResponse(e.getMessage());
        }
    }

    @Override
    public BaseResponse<RequirementInfo> updateRequirement(UpdateRequirementRequest request) {
        try {
            ...
            log.error("更新需求信息失败，需求ID: {}", request.getId(), e);
            return buildErrorResponse(e.getMessage());
        }
    }

    @Override
    public BaseResponse<Void> deleteRequirement(String requirementId) {
        try {
            String url = buildApiUrl("/workitems/" + requirementId);
            ...
            log.error("删除需求失败，需求ID: {}", requirementId, e);
            return buildErrorResponse(e.getMessage());
        }
    }

    @Override
    public BaseResponse<RequirementListResponse> listRequirements(RequirementQueryRequest request) {
        try {
            // 设置需求类型过滤
            if (request != null) {
                request.setType("Requirement");
    ...
    @Override
    public BaseResponse<RequirementInfo> updateRequirementStatus(UpdateStatusRequest request) {
        try {
            ...
            log.error("修改需求状态失败，需求ID: {}", request.getWorkItemId(), e);
            return buildErrorResponse(e.getMessage());
        }
    }

    @Override
    public BaseResponse<BatchUpdateStatusResponse> batchUpdateRequirementStatus(BatchUpdateStatusRequest request) {
        try {
